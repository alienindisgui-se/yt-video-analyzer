...
def summarize_comments_with_ai(title, comments_dict, v_id):
    ...
    # Extract comment texts. Limit to 150 comments to avoid hitting free-tier token limits easily.
    texts = [c['t'] for c in comments_dict.values() if not c.get('deleted', False)]
    if not texts:
        logging.info("No comments available to summarize.")
        return
        
    sample_texts = texts[:150]
    comments_string = "\n".join([f"- {t}" for t in sample_texts])
    
    prompt = (
        f"Analyze the following comments from the YouTube video '{title}'. "
        "Write a concise summary (around 3-4 sentences) of the general viewer sentiment, "
        "the main topics being discussed, and the overall opinions of the audience.\n\n"
        f"Comments:\n{comments_string}\n\n"
    ...
    try:
        ...
        if WEBHOOK:
            ...
            payload = {
                "embeds": [{
                    "title": f"🤖 AI Comment Analysis {current_date}",
                    "description": summary,
                    "color": 0x3498DB,
                    "fields": [
                        {"name": "Channel", "value": channel_name, "inline": True},
                        {"name": "Video", "value": f"[{title}](https://www.youtube.com/watch?v={v_id})", "inline": False}
                    ]
                }]
            }
...
def send_deletion_alert(author, text, v_id, ts, deleted_at, percentage, title):
    ...
    # Find channel name for this video
    channel_name = "Unknown Channel"
    for channel in CHANNELS:
        if v_id in [vid for vid in fetch_latest_videos([channel])]:
            channel_name = channel
